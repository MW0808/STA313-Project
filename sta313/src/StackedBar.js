...
function StackedBar({ data }) {
  ...
  useEffect(() => {
    ...
    const getOffset = (d, i, category) => {
  if (!baselineCategory) return 0;

  // Find the baseline series
  const baselineSeries = series.find(s => s.key === baselineCategory);

  // The baseline's lower bound for this month
  const baselineStart = baselineSeries[i][0];

  // If this bar belongs to the baseline category, shift it so it starts at 0
  if (category === baselineCategory) {
    return -y(baselineStart);
  }

  // For all other categories, shift them so their bottom aligns above the baseline
  return -y(baselineStart);
};
    ...
    rects.join("rect")
        ...
        .attr("y", d => y(d[1]) + getOffset(d, d.data.index, d.category))
